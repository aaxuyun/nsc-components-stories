...
class SupervisorUploadForm extends Component {
  constructor (props) {
    super(props)
    this.state = {
  ...
  beforeUpload = (file) => {
    ...
    getSTSClient().then(uploadClient => {
      ...
      console.error('SupervisorUploadForm error', e)
    ...
    return (
      <div>
        <Dragger {...props}>
          ...
          <p className="ant-upload-text">点击获取拖动 图片或文档 到这块区域完成附件上传</p>
        </Dragger>
      </div>
    )
  }
...
SupervisorUploadForm.propTypes = {
	onChange: PropTypes.func
}

SupervisorUploadForm.defaultProps = {

}

export default SupervisorUploadForm
